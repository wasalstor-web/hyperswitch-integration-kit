/** استدعاءات Hyperswitch Router (لوحة التحكم / مستخدم) — المفتاح السري لا يُعرَض للمتصفح */

export function hyperswitchBaseUrl(): string {
  const u = Deno.env.get("HYPERSWITCH_BASE_URL")?.trim().replace(/\/$/, "") ?? "";
  return u;
}
...
export async function hyperswitchGetUserMerchantId(
  base: string,
  bearerToken: string,
): Promise<string> {
